...
export const Series = () => {
  ...
  const grade = async () => {
    ...
    try {
      ...
      if (data.correct) {
        ...
        setFeedback(`❌ Casi, revisa el patrón con calma. ${data.feedback ?? ''}`);
  ...
  const mascotMessage = (() => {
    if (!totalSolved) return 'Observa cómo crecen los números y descubre el patrón 😊';
    if (lastCorrect === true) {
      if (streak >= 3)
        return '🔥 ¡Racha increíble! Ya dominas las series, sigamos.';
      return '✅ Muy bien, tu ojo para los patrones mejora en cada intento.';
    }
    if (lastCorrect === false) {
      return 'Si algo no cuadra, revisa cuánto aumenta o se multiplica cada paso 💛';
    }
    return 'Busca qué se repite: sumas, restas o multiplicaciones ✨';
  })();
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.banner}>
        <View>
          <Text style={styles.appTitle}>Series</Text>
        </View>
      ...
      {problem ? (
        <>
          ...
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Completa la serie</Text>

            <Text style={styles.questionText}>
              {problem.questionText ?? 'Observa la secuencia y completa el número faltante.'}
            </Text>

            {/* INPUT GRANDE PARA RESPUESTA (mismo estilo general que la burbuja de suma) */}
            <View style={styles.seriesInputContainer}>
              <Text style={styles.seriesLabel}>Respuesta</Text>
              <TextInput
            ...
            {showHint && (
              <View style={styles.hintBox}>
                <Text style={styles.hintText}>
                  Analiza la diferencia entre los números de la serie. 
                  ¿Aumentan siempre lo mismo (aritmética) o se multiplican (geométrica)?
                  Observa el patrón paso a paso antes de responder.
                </Text>
              </View>
            )}
...
const styles = StyleSheet.create({
  // FONDO GENERAL (igual que Addition)
  container: {
    flex: 1,
  ...
  // INPUT PARA SERIES (mismo mood que la burbuja de suma)
